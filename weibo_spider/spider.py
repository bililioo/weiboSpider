...
class Spider:
    ...
    def write_recommend(self, recommends):
        """将爬取到的信息流写入文件或数据库"""
        for writer in self.writers:
            writer.write_recommend(recommends)

    def write_comment(self, comments):
        """将爬取到的信息流写入文件或数据库"""
        for writer in self.writers:
            writer.write_comment(comments)
    ...
    def get_recommend_list(self):
        """获取热门信息流"""
        recommend = RecommendParser(self.cookie, self.filter, self.tab_id).handleContents()
        return recommend
